import React from "react";

import { Grid, MenuItem } from "@mui/material";

import { FormProvider, UseFormReturn } from "react-hook-form";
import { useTranslation } from "react-i18next";

import FormFileInput from "src/components/form/FormFileInput";
import { FormSelectInput } from "src/components/form/FormSelectInput";
import { FormTextInput } from "src/components/form/FormTextInput";

import { CorporateUpdateSchemaType } from "../validation/updateCorporate";

export { CorporateUpdateForm };

interface CorporateUpdateFormProps {
  form: UseFormReturn<CorporateUpdateSchemaType>;
}

function CorporateUpdateForm({
  form,
}: CorporateUpdateFormProps): React.ReactElement {
  const { t } = useTranslation();
  return (
    <FormProvider {...form}>
      <Grid container spacing={1}>
        <Grid item xs={12}>
          <FormFileInput
            placeholder={t("labels:placeholder.image")}
            name={"logo"}
          />
        </Grid>
        <Grid item xs={12}>
          <FormTextInput
            sx={{ width: "100%" }}
            name={"name"}
            label={t("labels:name.special")}
          />
        </Grid>
        <Grid item xs={6}>
          <FormTextInput
            sx={{ width: "100%" }}
            name={"social_reason"}
            label={t("labels:social_reason")}
          />
        </Grid>
        <Grid item xs={6}>
          <FormTextInput
            sx={{ width: "100%" }}
            name={"rfc"}
            label={t("labels:rfc")}
          />
        </Grid>
        <Grid item xs={6}>
          <FormTextInput
            sx={{ width: "100%" }}
            name={"tire_fee"}
            label={t("labels:tire_fee")}
            inputProps={{ type: "number" }}
          />
        </Grid>
        <Grid item xs={6}>
          <FormSelectInput
            name={"fee_currency_type"}
            label={t("labels:fee_field.label")}
          >
            <MenuItem value="MXN">{t("labels:fee_field.options.mxn")}</MenuItem>
            <MenuItem value="USD">{t("labels:fee_field.options.usd")}</MenuItem>
            <MenuItem value="EUR">{t("labels:fee_field.options.eur")}</MenuItem>
          </FormSelectInput>
        </Grid>
      </Grid>
    </FormProvider>
  );
}
