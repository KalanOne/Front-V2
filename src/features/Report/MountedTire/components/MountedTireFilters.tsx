...
import { MountedTireFilterSchemaType } from "../validation/filterMountedTire";

export { MountedTireFilters };

interface MountedTireFiltersProps {
  selectedFilters: Partial<Record<keyof MountedTireFilterSchemaType, string>>;
}

function MountedTireFilters({ selectedFilters }: MountedTireFiltersProps) {
      ...
      <Box sx={{ flexGrow: 1 }}>
        <Grid2 container spacing={2}>
          ...
          <Grid2 xs={12} md={6}>
            ...
            {selectedFilters.vehicle_types && (
              <FilteredItem
                title={t("general:warehouses")}
                value={selectedFilters.vehicle_types}
              />
            )}
          </Grid2>
        </Grid2>
